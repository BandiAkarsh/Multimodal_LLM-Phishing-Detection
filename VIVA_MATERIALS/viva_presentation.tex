...
\begin{frame}{4-Category Classification}
    \begin{center}
        \begin{tikzpicture}[scale=0.8]
            \node[draw, fill=blue!20, rounded corners, minimum width=3cm, minimum height=1cm] (legit) at (0,0) {LEGITIMATE};
            \node[draw, fill=red!20, rounded corners, below of=legit, node distance=1.5cm] (phish) {PHISHING};
            \node[draw, fill=orange!20, rounded corners, right of=phish, node distance=3cm] (ai) {AI-GENERATED};
            \node[draw, fill=purple!20, rounded corners, left of=phish, node distance=3cm] (kit) {PHISHING KIT};
            
            \draw[->, thick] (legit) -- (phish) node[midway, right] {Static ML};
            \draw[->, thick] (phish) -- (ai) node[midway, above] {MLLM check};
            \draw[->, thick] (phish) -- (kit) node[midway, above] {Toolkit sig};
        \end{tikzpicture}
    \end{center}
\end{frame}
