...
feature_extraction = load_module(
    "feature_extraction", os.path.join(project_root, "05_utils/feature_extraction.py")
)
mllm_transformer = load_module(
    "mllm_transformer", os.path.join(project_root, "05_utils/mllm_transformer.py")
)
typosquatting_detector = load_module(
    "typosquatting_detector", os.path.join(project_root, "05_utils/typosquatting_detector.py")
)
...
class PhishingDetectionService:
        ...
        try:
            ...
            if scrape_success:
                ...
                proof = {
                    ...
                    "screenshot_size": (
                        scrape_result.get("screenshot").size
                ...
                if typosquat_result.get("requires_content_verification") and page_title:
                    ...
                    if typosquat_result.get("content_verified") and not typosquat_result.get(
                        "is_typosquatting"
                    ):
                        print(
                            f"   [CONTENT OK] {typosquat_result.get('verification_reason', 'Content verified')}"
                        )
    ...
    def _analyze_scraped_content_4cat(
        self,
        url: str,
        scrape_result: dict,
        ...
        if toolkit_signatures.get("detected"):
            ...
            explanation = (
                f"Phishing toolkit detected: {toolkit_name}. "
    ...
    def _lightweight_ai_detection(
        self, text_content: str, html_summary: dict
    ) -> Tuple[float, list]:
    ...
    def _analyze_traditional_phishing(
        self,
        url: str,
        scrape_result: dict,
        typosquat_result: dict,
        ...
        if typosquat_result.get("is_typosquatting") and not typosquat_result.get(
            "content_verified"
        ):
            method = typosquat_result.get("detection_method")
            if method not in ["faulty_extension", "invalid_extension"]:
                risk_score += 60
                risk_factors.append(
                    f"Brand impersonation: {typosquat_result.get('impersonated_brand')}"
                )
    ...
    def _analyze_static_fallback(self, url: str, force_mllm: bool = False) -> dict:
        ...
        risk_score = self._calculate_risk_score(
            url_features, typosquat_result, ml_prediction, ml_confidence
        )
        ...
        if typosquat_result.get("is_typosquatting"):
            ...
            recommended_action = (
                "block"
                if (classification == "phishing" and confidence >= 0.8)
                else ("warn" if classification == "phishing" else "allow")
            )
        else:
    ...
    def _create_typosquat_result(
        self, url: str, typosquat_result: dict, offline: bool = False
    ) -> dict:
    ...
    def _calculate_risk_score(
        self, features: dict, typosquat: dict = None, ml_pred: int = None, ml_conf: float = 0.5
    ) -> float:
