...
def send_desktop_notification(title, message, timeout=10):
    ...
    try:
        subprocess.run(
            ["notify-send", "-a", "Phishing Guard", "-u", "critical", title, message], check=False
        )
                    ...
                    if urls:
                        ...
                        for url in urls:
                            ...
                            if res["classification"] != "legitimate":
                                ...
                                if (
                                    not highest_threat or "KIT" in cat
                                ):  # Prioritize KIT > AI > PHISH
                                    highest_threat = cat
                        if found_phish:
...
async def scan_file(service, file_path, is_online):
    ...
    for url in urls:
        res = await service.analyze_url_async(url, force_mllm=is_online)
        status = (
            f"{Colors.RED}[{res['classification'].upper()}]{Colors.END}"
            if res["classification"] != "legitimate"
            else f"{Colors.GREEN}[SAFE]{Colors.END}"
        )
...
async def main():
    ...
    parser.add_argument(
        "--daemon", action="store_true", help="Run in background mode (silences output)"
    )
