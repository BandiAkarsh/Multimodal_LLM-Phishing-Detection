...
class URLSecurityValidator:
    ...
    DANGEROUS_CHARS: Set[str] = {"<", ">", '"', "'", "`", "{", "}", "|", "\\", "^", "\x00", "\x01"}
    ...
    def validate(self, url: str, strict: bool = True) -> Tuple[bool, List[str]]:
        ...
        if scheme not in self.ALLOWED_SCHEMES:
            self.validation_errors.append(
                f"Scheme '{scheme}' is not in allowed list: {self.ALLOWED_SCHEMES}"
            )
            if strict:
                return False, self.validation_errors
        ...
        if len(hostname) > self.MAX_DOMAIN_LENGTH:
            self.validation_errors.append(
                f"Domain name too long: {len(hostname)} > {self.MAX_DOMAIN_LENGTH}"
            )
            return False, self.validation_errors
        ...
        if self._is_private_host(hostname):
            self.validation_errors.append(
                "URL resolves to private/internal IP address (SSRF protection)"
            )
            return False, self.validation_errors
        ...
        if parsed.path:
            ...
            if len(decoded_path) > self.MAX_PATH_LENGTH:
                self.validation_errors.append(
                    f"Path too long: {len(decoded_path)} > {self.MAX_PATH_LENGTH}"
                )
                return False, self.validation_errors
                ...
                for key, values in params.items():
                    for value in values:
                        if any(char in key or char in value for char in self.DANGEROUS_CHARS):
                            self.validation_errors.append(
                                f"Suspicious characters in query parameter: {key}"
                            )
                            if strict:
                                return False, self.validation_errors
            except Exception:
