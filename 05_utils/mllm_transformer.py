...
class MLLMFeatureTransformer:
    ...
    def classify_threat(
        self, metadata: Dict[str, Any], toolkit_signatures: Optional[Dict] = None
    ) -> Tuple[ThreatCategory, float, str]:
        """
    ...
    def _detect_ai_generated_content(
        ...
        if dom_structure:
            ...
            if has_login and num_links < 5 and num_forms <= 2:
                detected_indicators.append(
                    "Minimal page with login form (potential phishing landing)"
                )
                score += 0.2
...
"""
    ...
    def _create_classification_prompt(self, metadata: Dict[str, Any], ai_indicators: list) -> str:
        ...
        feature_desc = "\n".join(
            [f"  - {k}: {v}" for k, v in features.items() if k != "typosquatting"]
        )
        ...
        if ai_indicators:
            ai_indicator_desc = "Detected AI-Content Indicators:\n" + "\n".join(
                [f"  - {ind}" for ind in ai_indicators]
            )
...
Respond with the JSON classification only."""
    ...
    def _parse_classification_response(
        self, response: str, ai_score: float
    ) -> Tuple[ThreatCategory, float, str]:
        """
        ...
        if "legitimate" in response_lower and "not" not in response_lower[:50]:
            ...
            "ai_generated" in response_lower or "ai-generated" in response_lower or ai_score >= 0.5
        ):
