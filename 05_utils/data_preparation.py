...
class DataPreprocessor:
    ...
    async def process_dataset(self, csv_path, max_samples=None):
        """
        ...
        async with WebScraper(headless=True) as scraper:
            for idx, row in tqdm(df.iterrows(), total=len(df), desc="Processing URLs"):
                ...
                if scraped_data["success"]:
                    ...
                    results.append({"idx": idx, "url": url, "label": label, "success": True})
                else:
                    results.append({"idx": idx, "url": url, "label": label, "success": False})
