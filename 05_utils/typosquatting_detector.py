...
PROTECTED_BRANDS = {
    ...
    "paypal": {
        "domains": ["paypal.com"],
        "industry": "payment",
        "keywords": ["payment", "money", "transfer"],
    },
    ...
    "kotak": {
        "domains": ["kotak.com", "kotakbank.com", "kotak.bank", "kotak.bank.in"],
        "industry": "banking",
        "keywords": ["bank", "mahindra", "netbanking"],
    },
    ...
    "gpay": {
        "domains": ["pay.google.com"],
        "industry": "payment",
        "keywords": ["google", "payment"],
    },
    "razorpay": {
    ...
    "google": {
        ...
        "industry": "tech",
        "keywords": ["search", "mail", "account"],
    },
    "microsoft": {
        "domains": ["microsoft.com", "outlook.com", "live.com", "office.com"],
        "industry": "tech",
        "keywords": ["windows", "office", "account"],
    },
    "apple": {
        "domains": ["apple.com", "icloud.com"],
        "industry": "tech",
        "keywords": ["iphone", "mac", "icloud"],
    },
    "amazon": {
        "domains": ["amazon.com", "amazon.in", "aws.amazon.com"],
        "industry": "ecommerce",
        "keywords": ["shop", "prime", "order"],
    },
    "facebook": {
    ...
    "linkedin": {
        ...
        "keywords": ["job", "network"],
    },
    "netflix": {
        "domains": ["netflix.com"],
        "industry": "streaming",
        "keywords": ["stream", "watch", "movie"],
    },
    "spotify": {
        "domains": ["spotify.com"],
        "industry": "streaming",
        "keywords": ["music", "stream"],
    },
    # E-commerce
    ...
    "fedex": {
        "domains": ["fedex.com"],
        "industry": "shipping",
        "keywords": ["delivery", "package", "track"],
    },
    "ups": {
        "domains": ["ups.com"],
        "industry": "shipping",
...
class TyposquattingDetector:
    ...
    def analyze(self, url: str, page_content: Optional[Dict] = None) -> dict:
        ...
        if not suffix and domain in self.tld_typos:
            ...
            results["details"].append(
                f"Faulty or incorrect extension: '.{fake_tld}' (Did you mean '.{intended_tld}'?)"
            )
            return results
        ...
        if suffix in self.tld_typos:
            ...
            results["details"].append(
                f"Faulty or incorrect extension: '.{suffix}' (Did you mean '.{intended_tld}'?)"
            )
            return results
        ...
        if "." not in url.split("//")[-1]:
            if url.split("//")[-1] != "localhost":
                ...
                results["details"].append(
                    f"Invalid domain structure: '{url}'. Missing valid extension."
                )
                return results
        ...
        if suffix:
            ...
            # Check if it's a two-part TLD where both parts are valid
                ...
                if suffix_parts[0] in self.valid_tlds and suffix_parts[1] in self.valid_tlds:
                    ...
                    if (
                        suffix_parts[0] not in self.valid_tlds
                        and suffix_parts[1] not in self.valid_tlds
                    ):
                        results["is_typosquatting"] = True
                        ...
                        results["details"].append(
                            f"Invalid/Non-existent extension '.{suffix}' detected."
                        )
                        return results
            else:
        ...
        for brand, brand_info in self.brands.items():
            legitimate_domains = (
                brand_info["domains"] if isinstance(brand_info, dict) else brand_info
            )
            ...
            if brand in domain:
                ...
                results["details"].append(
                    f"Contains '{brand}' but is not a legitimate {brand} domain"
                )
            ...
            if similarity > 0.7 and similarity < 1.0:
                ...
                results["details"].append(
                    f"Domain '{domain}' is {similarity*100:.1f}% similar to '{brand}'"
                )
    ...
    def verify_with_content(
        self, typosquat_result: dict, page_title: str, page_text: str = ""
    ) -> dict:
        """
        ...
        if found_legitimate and not industry_match:
            ...
            typosquat_result["verification_reason"] = (
                f"Page appears to be a legitimate {title_lower.split()[0] if title_lower else 'business'}, not {brand}"
            )
            typosquat_result["risk_increase"] = 0
            typosquat_result["details"] = [
                f"Content verification passed: Not related to {brand} {expected_industry}"
            ]
        else:
            typosquat_result["content_verified"] = True
            if industry_match:
                typosquat_result["verification_reason"] = (
                    f"Page content matches {brand} {expected_industry} - likely impersonation"
                )
