...
class URLFeatureExtractor:
    ...
    def _get_idn_features(domain: str) -> Dict[str, Any]:
        ...
        features["unicode_letter_count"] = sum(
            categories.get(cat, 0) for cat in ["Lu", "Ll", "Lt", "Lm", "Lo"]
        )
        ...
        features["unicode_symbol_count"] = sum(
            categories.get(cat, 0) for cat in ["Sm", "Sc", "Sk", "So"]
        )
        features["unicode_punctuation_count"] = sum(
            categories.get(cat, 0) for cat in ["Pc", "Pd", "Ps", "Pe", "Pi", "Pf", "Po"]
        )
    ...
    def _extract_url_pattern_features(url: str, parsed) -> Dict[str, Any]:
        ...
        features["is_shortened_url"] = (
            1 if any(pattern in url.lower() for pattern in shortening_patterns) else 0
        )
    ...
    def _extract_tls_features(url: str) -> Dict[str, Any]:
        ...
        if extract_tls_features:
            try:
                ...
                features["cert_expiring_soon"] = (
                    1 if cert_days is not None and 0 <= cert_days < 7 else 0
                )
            except Exception:
