...
allowed_origins = os.getenv(
    "ALLOWED_ORIGINS", "http://localhost:3000,http://localhost:8080,http://127.0.0.1"
).split(",")
...
async def generate_api_key(
    name: str, description: str = "", user: dict = Depends(get_current_user)
):
    """
...
async def check_connectivity():
    ...
    return {
        ...
        "message": (
            "Full multimodal scraping available"
            if connectivity["is_online"]
            else "Using static URL analysis (less accurate)"
        ),
    }
...
async def batch_analyze_urls(
    ...
    **Authentication required:** Include JWT token in Authorization header.
    ...
    for url in request.urls:
        ...
        if not is_valid:
            raise HTTPException(
                status_code=400, detail=f"URL validation failed for {url}: {error_msg}"
            )
...
async def extract_features(
    ...
    **Authentication required:** Include JWT token in Authorization header.
    ...
    if not is_valid:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=f"URL validation failed: {error_msg}"
        )
