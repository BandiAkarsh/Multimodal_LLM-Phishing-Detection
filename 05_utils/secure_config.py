...
class SecureConfigManager:
    ...
    def _get_or_create_key(self) -> bytes:
        ...
        try:
            keyring.set_password(
                self.app_name, "master_key", base64.urlsafe_b64encode(key).decode()
            )
        except Exception:
            # Fallback to file
    ...
    def decrypt_config(self) -> dict:
        ...
        if not self.config_file.exists():
            raise FileNotFoundError(
                f"No encrypted config found at {self.config_file}. " "Run setup first."
            )
    ...
    def rotate_key(self) -> None:
        ...
        try:
            keyring.set_password(
                self.app_name, "master_key", base64.urlsafe_b64encode(new_key).decode()
            )
        except:
            with open(self.key_file, "wb") as f:
                f.write(new_key)
